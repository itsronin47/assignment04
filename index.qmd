...
This graph identifies the 7 wealthiest states based on average family income. Each point has the number state on top of it, so we can identify which 7 states are the wealthiest. We will use the FIPS codes of those states to inform our next visualization.
\newpage
...
```
This chart shows how many respondents with food insecurity score of 10 each of the 7 wealthiest states has. The dotted line shows, by comparison, the average number of respondents with a food insecurity score of 10 across all other states in the US.
\newpage
...
geom_text(aes(x = rich_state_codes, y = percents,
  ...
  labs(
    title = "Percent of Respondents with Food Insecurity Score = 10",
